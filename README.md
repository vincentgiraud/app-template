...
| Research Azure stack + pricing | 1–2 days | 20 min (review `@az-saas-planner` output) |
| Set up CI/CD with security scanning | 4–8 hours | Already done |
| Write compliance + security docs | 1–2 days | Generated by stack planner agents |
...
> Requires a project first — run Option A, or manually create your `package.json` / `pyproject.toml`.

```bash
pnpm install       # or npm install / uv sync
...
| `backend-designer` | API conventions, database modeling rules, auth flows |
| `feature-planner` | Post-bootstrap feature design + GitHub Issue creation |
...
| Agent | Role |
|-------|------|
| `@Frontend Developer` | React/TypeScript feature implementation |
| `@Test Writer` | Test coverage and quality |
| `@Documentation Updater` | Documentation sync with code changes |
...
| Workflow | Steps |
|----------|-------|
| `ci.yml` | Lint → Type check → Test + Security scan (parallel) → Build → Quality gate |
| `deploy.yml` | Staging deploy → Smoke test → Production deploy (via `azd`) |

### Compliance & Security Documentation

The `docs/stack-report.md` is **generated by the agent pipeline** when you run `@Spec Planner` (which auto-chains to `@az-saas-planner`). It consolidates:

- Compliance frameworks (GDPR, SOC 2, HIPAA, etc. — based on your industry/region)
- Security controls and tooling recommendations
- Azure service comparison across 3 tiers (cost-optimized, recommended, enterprise)
- Growth projections and scaling cliffs

Architecture, frontend, backend, and task specs live in `.github/instructions/` where Copilot loads them automatically.
...
├── docs/
│   └── README.md            # stack-report.md generated here by agent pipeline
...
| **7. Reflect** | Update compliance and security posture | Review docs quarterly | docs-updater agent keeps generated docs current |
...
| File | What to change |
|------|---------------|
| `.env.example` → `.env` | Copy and add your [fine-grained PAT](https://github.com/settings/tokens?type=beta) with actions, contents, issues, pull-requests permissions |
| `.github/ISSUE_TEMPLATE/config.yml` | Replace `OWNER/REPO` with your actual GitHub path |
| `docs/` | Generated by `@Spec Planner` — no manual setup needed |
